#!/usr/bin/env python3
"""
Simple backend server that runs without complex AI dependencies
This provides a working backend for frontend development
"""
...
async def health_check():
    return {
        ...
        "message": "Running in mock mode - AI components not available"
    }
...
async def generate_article(request: GenerationRequest):
    """Generate an article (mock implementation)"""
    article_id = str(uuid.uuid4())
    
    # Generate citations based on article length and max sources
...
async def validate_article(article_id: str):
    """Validate an article (mock implementation)"""
...
if __name__ == "__main__":
    ...
    print("⚠️  Note: Running in mock mode - AI components not available")
    print("   To enable real AI features, ensure all dependencies are installed")
